...
const MyProducts = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-900 via-zinc-800 to-gray-900 text-white px-2 py-4 sm:px-8 sm:py-6 flex justify-center">
      {alert && (
        <Alert
          type={alert.type}
          title={alert.title}
      ...
      <div className="w-full max-w-6xl bg-zinc-900/50 rounded-2xl px-3 sm:px-10 py-4 sm:py-6 shadow-xl border border-zinc-700">
        {/* Header */}
        ...
        {/* Filters & Add Button */}
        <div className="bg-zinc-800/40 rounded-lg border border-zinc-700 p-3 sm:p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
              <div className="flex-1 sm:flex-initial flex items-center justify-between gap-2 bg-zinc-800 px-3 py-2 rounded-lg border border-zinc-700 min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  <Filter className="w-4 h-4 text-gray-400 shrink-0" />
                  <select 
                    value={filterStatus} 
                    onChange={(e) => setFilterStatus(e.target.value)} 
                    className="bg-transparent focus:outline-none text-white text-sm cursor-pointer w-full"
                  >
              ...
              <Link 
                to="/seller/add-book" 
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 rounded-lg hover:from-yellow-500 hover:to-yellow-600 transition-all font-bold text-sm whitespace-nowrap shrink-0"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredAndSortedBooks.map((book) => {
              ...
              return (
                <div 
                  key={book._id} 
                  className="bg-zinc-800/40 rounded-lg border border-zinc-700 overflow-hidden hover:border-yellow-400/50 transition-all group flex flex-row sm:flex-col shadow-xl"
                >
                  {/* Left / Top Image Container */}
                  <div className="relative w-28 sm:w-full h-auto sm:h-40 bg-zinc-900 overflow-hidden shrink-0">
                    <img 
                      src={book.url || book.images?.[0]?.url} 
                      alt={book.title} 
                      className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500 min-h-[140px] sm:min-h-0" 
                    />
                        ...
                        <div className={`flex items-center gap-1 ${stockInfo.bg} rounded-full px-2 py-1 group/stock`}>
                          ...
                          {book.productStatus === "Available" && (
                            <button
                              ...
                              className="p-0.5 hover:bg-white/10 rounded opacity-0 sm:opacity-0 group-hover/stock:opacity-100 focus:opacity-100"
                            >
                    ...
                    <div className="absolute bottom-2 left-2">
                      <span className="text-[10px] sm:text-xs px-2 py-0.5 bg-black/70 rounded text-gray-200">
                  ...
                  <div className="p-3 sm:p-4 flex-1 flex flex-col justify-between min-w-0">
                    <div>
                      <h3 className="font-bold text-xs sm:text-sm text-white mb-1 line-clamp-2 group-hover:text-yellow-400 transition-colors">
                        {book.title}
                      </h3>
                      <p className="text-gray-400 text-[10px] sm:text-xs mb-2 truncate">by {book.author}</p>

                      {/* Stats Grid */}
                      <div className="grid grid-cols-3 gap-1.5 sm:gap-2 mb-3 text-[10px] sm:text-xs">
                        <div className="bg-purple-500/10 rounded p-1.5 sm:p-2 border border-purple-500/20 text-center">
                          <p className="text-purple-400 mb-0.5">Views</p>
                          <p className="font-bold text-white text-xs sm:text-sm">{book.views || 0}</p>
                        </div>
                        <div className="bg-green-500/10 rounded p-1.5 sm:p-2 border border-green-500/20 text-center">
                          <p className="text-green-400 mb-0.5">Sold</p>
                          <p className="font-bold text-white text-xs sm:text-sm">{book.sold || 0}</p>
                        </div>
                        <div className={`${stockInfo.bg} rounded p-1.5 sm:p-2 border border-${stockInfo.color.replace('text-', '')}/20 text-center`}>
                          <p className={stockInfo.color + " mb-0.5"}>Stock</p>
                          <p className="font-bold text-white text-xs sm:text-sm">{book.stock || 0}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between mb-3 pb-2 border-b border-zinc-700">
                        <div>
                          ...
                          <p className="text-sm sm:text-base font-bold text-yellow-400">₹{book.price.toLocaleString()}</p>
                        </div>
                        {book.sold > 0 && (
                          <div className="text-right">
                            <p className="text-[10px] text-gray-500 mb-0.5">Est. Earned</p>
                      ...
                      <div className="flex gap-2">
                        <Link 
                          ...
                          className="flex-1 flex items-center justify-center gap-1 py-2 sm:py-2.5 bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 rounded-lg hover:from-yellow-500 hover:to-yellow-600 transition-all text-xs font-bold active:scale-[0.98]"
                        >
                        ...
                        <button 
                          ...
                          className="px-2.5 sm:px-3 py-2 sm:py-2.5 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500/20 transition-all border border-blue-500/20 active:scale-[0.98]"
                        >
                          <RefreshCw className="w-3.5 h-3.5" />
                        </button>
                        <button 
                          onClick={() => setDeleteConfirm(book._id)} 
                          className="px-2.5 sm:px-3 py-2 sm:py-2.5 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition-all border border-red-500/20 active:scale-[0.98]"
                        >
