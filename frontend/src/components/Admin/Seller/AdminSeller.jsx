...
const DetailModal = ({ seller, onClose, onAction }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-zinc-900 rounded-2xl max-w-4xl w-full border border-zinc-700 shadow-2xl my-8 animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex flex-wrap gap-3 pt-4 border-t border-zinc-700">
            {seller.status === "Approved" && (
              <>
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-black px-6 py-3 rounded-lg font-bold transition-all shadow-lg flex items-center justify-center gap-2 transform hover:scale-[1.02] duration-300"
                >
                  <FaClock />
                  View Orders
                </button>
                <button
                  onClick={() => {
                    const sellerUserId = seller.user?._id || seller.user;
                    navigate(`/Admin/Seller-Products?sellerId=${sellerUserId}&sellerName=${encodeURIComponent(seller.fullName)}`);
                  }}
                  className="flex-1 bg-gradient-to-r from-zinc-700 to-zinc-600 hover:from-zinc-600 hover:to-zinc-500 text-white border border-zinc-500 px-6 py-3 rounded-lg font-bold transition-all shadow-lg flex items-center justify-center gap-2 transform hover:scale-[1.02] duration-300"
                >
                  <FaStore />
                  View Products
                </button>
            ...
            {seller.status === "Approved" && (
              <button
                onClick={() => onAction(seller, "ban")}
                className="bg-gradient-to-r from-slate-600 to-gray-700 hover:from-slate-700 hover:to-gray-800 text-white px-6 py-3 rounded-lg font-semibold transition-all shadow-lg"
              >
...
const AllSellers = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-900 via-zinc-800 to-gray-900 text-white px-4 sm:px-8 py-10 flex justify-center animate-in fade-in duration-300">
      {alert && (
        <Alert
          type={alert.type}
          title={alert.title}
      ...
      <div className="w-full max-w-7xl bg-zinc-900/50 rounded-3xl px-6 sm:px-12 py-10 shadow-xl border border-zinc-700">
        ...
        {!filteredSellers ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-in zoom-in-95 duration-300">
