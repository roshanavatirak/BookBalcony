...
//   useEffect(() => {
//     window.scrollTo(0, 0);
//     console.log("🔍 [INIT] Component mounted with order:", order);
//     console.log("🔍 [INIT] Address provided:", address);
//     fetchBookDetails();
//   }, []);
...
//       script.onerror = () => console.error("❌ [RAZORPAY] Failed to load SDK");
//       document.body.appendChild(script);
//     }
//   }, []);

//   // ✅ CRITICAL: Fetch complete book details including seller
//   const fetchBookDetails = async () => {
//     try {
//       console.log("📚 [FETCH] Starting to fetch book details...");
//       console.log("📚 [FETCH] Order items:", order.items);
      
//       const bookIds = order.items.map(item => item._id);
//       console.log("📚 [FETCH] Book IDs to fetch:", bookIds);
      
//       const bookDetailsPromises = bookIds.map(id => {
//         console.log(`📚 [FETCH] Fetching book ID: ${id}`);
//         return axios.get(`http://localhost:3000/api/v1/get-book-by-id/${id}`, {
//           headers: {
//             id: localStorage.getItem("id"),
//             authorization: `Bearer ${localStorage.getItem("token")}`,
//           }
...
//       const completeBooks = responses.map(res => {
//         console.log("📚 [FETCH] Book response:", res.data);
//         return res.data.data;
//       });
      
//       console.log("✅ [FETCH] Successfully fetched books with sellers:", completeBooks);
//       console.log("🔍 [FETCH] Seller info for each book:");
//       completeBooks.forEach((book, idx) => {
//         console.log(`   Book ${idx + 1}: ${book.title}`);
//         console.log(`   - seller field: ${book.seller}`);
//         console.log(`   - addedby field: ${book.addedby}`);
//         console.log(`   - Using: ${book.seller || book.addedby || 'MISSING'}`);
//       });
      
//       setBooksWithSellers(completeBooks);
//     } catch (err) {
//       console.error("❌ [FETCH] Error fetching book details:", err);
//       console.error("❌ [FETCH] Error response:", err.response?.data);
//       console.warn("⚠️ [FETCH] Falling back to original order items");
//       setBooksWithSellers(order.items);
//     }
//   };

//   const applyCoupon = () => {
//     console.log("🎟️ [COUPON] Attempting to apply coupon:", coupon);
//     if (validCoupons.includes(coupon.trim().toUpperCase())) {
//       setDiscountApplied(50);
//       setCouponMessage("✅ Coupon applied successfully! ₹50 off.");
//       console.log("✅ [COUPON] Valid coupon applied. Discount: ₹50");
//     } else {
//       setDiscountApplied(0);
//       setCouponMessage("❌ Invalid coupon code. Try again.");
//       console.log("❌ [COUPON] Invalid coupon code");
//     }
//   };

//   const validateAndFormatAddress = (address) => {
//     console.log("📍 [ADDRESS] Validating address:", address);
...
//     console.log("📍 [ADDRESS] Formatted address:", formatAddress);
...
//     if (missingFields.length > 0) {
//       console.warn("⚠️ [ADDRESS] Missing fields:", missingFields);
//     } else {
//       console.log("✅ [ADDRESS] Address validation passed");
//     }
...
//     console.log("💳 [PAYMENT] ========== STARTING ONLINE PAYMENT PROCESS ==========");
//     setIsProcessing(true);

//     try {
//       // Step 1: Validate Address
//       console.log("📍 [PAYMENT] Step 1: Validating address...");
//       const { formatAddress, missingFields } = validateAndFormatAddress(address);
      
//       if (missingFields.length > 0) {
//         console.error("❌ [PAYMENT] Address validation failed");
//         alert(`Please complete your address details. Missing: ${missingFields.join(", ")}`);
//         setIsProcessing(false);
//         return;
//       }
//       console.log("✅ [PAYMENT] Address validation passed");

//       // Step 2: Check Razorpay SDK
//       console.log("📦 [PAYMENT] Step 2: Checking Razorpay SDK...");
//       if (!window.Razorpay) {
...
//       // Step 3: Prepare Books Data
//       console.log("📚 [PAYMENT] Step 3: Preparing books data...");
//       const booksToUse = booksWithSellers.length > 0 ? booksWithSellers : order.items;
//       console.log("📚 [PAYMENT] Books to use:", booksToUse);
//       console.log("📚 [PAYMENT] Book count:", booksToUse.length);
      
//       // Validate seller info for each book
//       const booksWithoutSeller = booksToUse.filter(book => !book.seller && !book.addedby);
//       if (booksWithoutSeller.length > 0) {
//         console.error("❌ [PAYMENT] Books missing seller info:", booksWithoutSeller);
//         alert("Some books are missing seller information. Please contact support.");
//         setIsProcessing(false);
//         return;
//       }
//       console.log("✅ [PAYMENT] All books have seller information");

//       // Step 4: Create Razorpay Order
//       console.log("💰 [PAYMENT] Step 4: Creating Razorpay order...");
//       const finalAmount = order.payable - discountApplied;
//       console.log("💰 [PAYMENT] Final amount:", finalAmount);
      
//       const { data } = await axios.post("http://localhost:3000/api/v1/payment/order", {
//         amount: finalAmount,
//       });
//       console.log("✅ [PAYMENT] Razorpay order created:", data.order);

//       // Step 5: Configure Razorpay Options
//       console.log("⚙️ [PAYMENT] Step 5: Configuring Razorpay options...");
//       const options = {
//         key: "rzp_test_NBQpcL6r3o5ntb",
//         amount: data.order.amount,
//         currency: "INR",
...
//   try {
//     // Step 6: Prepare Verification Payload
//     console.log("🔐 [VERIFY] Step 6: Preparing verification payload...");
    
//     // ✅ FIX: Create orders for ALL books, not just the first one
//     const orderDataArray = booksToUse.map((book, index) => {
//       const sellerValue = book.seller || book.addedby;
      
//       console.log(`📦 [VERIFY] Preparing order ${index + 1}:`, {
//         bookId: book._id,
...
//       if (!sellerValue) {
//         throw new Error(`Book "${book.title}" is missing seller information`);
//       }
...
//         paymentMethod: "RAZORPAY",
//         paymentStatus: "Success", // ✅ Set to Success immediately after payment
//         orderStatus: "Order Placed",
//         amountPayable: finalAmount / booksToUse.length, // ✅ Distribute amount evenly
//         discount: discountApplied / booksToUse.length, // Distribute discount
//         shippingAddress: formatAddress,
//       };
//     });
    
//     console.log("📦 [VERIFY] Order data array:", JSON.stringify(orderDataArray, null, 2));
    
//     // ✅ If single book, send single order format
//     // ✅ If multiple books, send array format
//     const verifyPayload = {
//       razorpay_order_id: response.razorpay_order_id,
//       razorpay_payment_id: response.razorpay_payment_id,
//       razorpay_signature: response.razorpay_signature,
...
//     console.log("🔐 [VERIFY] Verification payload:", JSON.stringify(verifyPayload, null, 2));

//     // Step 7: Verify Payment and Create Orders
//     console.log("🔐 [VERIFY] Step 7: Verifying payment and creating orders...");
//     const verifyResponse = await axios.post(
//       "http://localhost:3000/api/v1/payment/verify",
//       verifyPayload,
//       {
...
//     console.log("✅ [VERIFY] Payment verification response:", verifyResponse.data);
...
//         console.log("🎉 [ORDER] Orders created successfully!");
...
//         console.warn("⚠️ [ORDER] Payment verified but order creation failed");
        
//         setPaymentDetails({
//           title: booksToUse.map(i => i.title).join(", "),
//           price: finalAmount,
//           failed: true,
//           errorDetails: verifyResponse.data.errorDetails || "Order processing failed",
//         });
//         setShowSuccessPopup(true);
//       }
//     } else {
...
//     } catch (err) {
//       console.error("❌ [PAYMENT] Payment initialization error:", err);
...
//     if (missingFields.length > 0) {
//       console.error("❌ [COD] Address validation failed");
//       alert(`Please complete your address details. Missing: ${missingFields.join(", ")}`);
//       return;
//     }
    
//     console.log("✅ [COD] Proceeding to COD page");
//     setShowCODPage(true);
//   };

//   const handleCODBack = () => {
//     console.log("⬅️ [COD] Returning from COD page");
//     setShowCODPage(false);
//   };
...
export default function Step3_Payment({ address, order, onBack }) {
  ...
  const fetchBookDetails = async () => {
    try {
      const bookIds = order.items.map(item => item._id);
      const bookDetailsPromises = bookIds.map(id =>
        axios.get(`http://localhost:3000/api/v1/get-book-by-id/${id}`, {
          headers: {
            id: localStorage.getItem("id"),
            authorization: `Bearer ${localStorage.getItem("token")}`,
          }
